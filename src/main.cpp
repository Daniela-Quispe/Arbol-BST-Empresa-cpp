...
private:
    ...
    Nodo* insertar(Nodo* nodo, Empleado emp) {
        ...
        if (emp.codigo < nodo->dato.codigo) {
            ...
            cout << "El codigo ya existe en el arbol.\n";
        }
    ...
    // ?? AGREGA AQUÍ
    int obtenerNivel(Nodo* nodo, int codigo, int nivel) {
        if (nodo == nullptr) return -1;
        if (nodo->dato.codigo == codigo) return nivel;

        if (codigo < nodo->dato.codigo)
            return obtenerNivel(nodo->izquierdo, codigo, nivel + 1);
        else
            return obtenerNivel(nodo->derecho, codigo, nivel + 1);
    }
...
public:
    ...
    void mostrarNodosHoja() {
     ...
     void mostrarNivel(int codigo) {
    int nivel = obtenerNivel(raiz, codigo, 0);

    if (nivel != -1) {
        cout << "El empleado esta en el nivel: " << nivel << endl;
    } else {
...
int main() {
    ...
    do {
        ...
        cout << "9. Mostrar nivel de un empleado\n";
        cout << "0. Salir\n";
